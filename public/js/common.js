( function($) {
        ...
        $('.dropdown-menu .dropdown-item').mouseover(function(){
			var myClass = $(this).attr("class");
			var class_count = myClass.split(" ");
			//var imgsrc=$("."+class_count[1] +" img").attr('src'); 
			var imgsrc=$("."+class_count[1] +" img").attr('data-src'); 
			
			$('.img-menu-banner').attr('src', imgsrc);
		  });
...
function mcImgSlider(i) {
    ...
    lb.prototype = {
        c: function() {
            ...
            o = f[cb];
           // s = f.offsetHeight;
		    s = getSliderBackgroundSize().height;
          ...
          $('.bgLayer').css({
				width: getSliderBackgroundSize().widthpx,
				height: getSliderBackgroundSize().heightpx				
			})
        ...
        w: function(g) {
            for (var k = [], i = g > 8 ? o : Math.round(o / a.f), l = g > 8 ? 1 : a.f, f = 0; f < l; f++) {
				...
				e[B] = 'url("' + b.e[h]("src") + '") no-repeat -' + f * i + "px 0% / "+getSliderBackgroundSize().widthpx +' '+ getSliderBackgroundSize().heightpx
...
function getSliderBackgroundSize(){

			var orgWidth = 1920.00;
            var orgHeigth = 780.00;
			var ratio = orgWidth / orgHeigth;
			var currentWidth  = Math.round(jQuery(document).width());
			var newHeight = Math.round(currentWidth / ratio);
			
			$('#slider, #slider .sliderInner').css('height',newHeight+'px')
